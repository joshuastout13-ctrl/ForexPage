# Package B — Production Restoration & Verification Plan

**Target Production Supabase Project:** `julhldzkiqdeuuoqmvlo` (`https://4xtrack.com`)  
**Canonical Migration Artifact:** `docs/proposed_withdrawal_concurrency_control_migration.sql`  
**Declared Version:** `2.1.0`  
**Canonical Git Blob ID:** `8b242f6ae3cd88050b8e801e342187629462f232`  
**Authoritative LF SHA-256:** `cd83dc116bcc51d7ff704bacd90764a85b370fe4e2d567323d2689e24270ad77`  
**Execution Guardrail:** READ-ONLY PREFLIGHT REQUIRED BEFORE MIGRATION EXECUTION. DO NOT RUN UNTIL SEPARATELY AUTHORIZED.

---

## Section A: Pre-Install Read-Only Production Preflight

Execute this block in the Supabase SQL Editor (`julhldzkiqdeuuoqmvlo`) immediately prior to migration to verify baseline preconditions:

```sql
-- ============================================================================
-- PREFLIGHT CHECK 1: TOTAL RECORD COUNT & POPULATION INTEGRITY
-- ============================================================================
SELECT 
  COUNT(*) AS total_withdrawals,
  COUNT(CASE WHEN investor_id IS NULL OR TRIM(investor_id) = '' THEN 1 END) AS missing_investor_id,
  COUNT(CASE WHEN account_id IS NULL OR TRIM(account_id) = '' THEN 1 END) AS missing_account_id,
  COUNT(CASE WHEN effective_accounting_date IS NULL THEN 1 END) AS null_effective_date_count,
  COUNT(CASE WHEN year IS NULL OR month_number IS NULL OR month_number < 1 OR month_number > 12 THEN 1 END) AS invalid_period_count,
  COUNT(CASE WHEN amount <= 0.00 THEN 1 END) AS non_positive_amount_count
FROM withdrawals;

-- ============================================================================
-- PREFLIGHT CHECK 2: EXACT STATUS CENSUS
-- ============================================================================
SELECT 
  COALESCE(status, 'NULL') AS status,
  COUNT(*) AS row_count,
  SUM(amount) AS total_amount,
  COUNT(CASE WHEN amount <= 0.00 THEN 1 END) AS non_positive_count
FROM withdrawals
GROUP BY status
ORDER BY status;

-- ============================================================================
-- PREFLIGHT CHECK 3: PACKAGE B DB OBJECT INVENTORY (EXPECTED ALL MISSING)
-- ============================================================================
SELECT 'column: idempotency_key' AS object_name, 
  EXISTS(SELECT 1 FROM information_schema.columns WHERE table_name='withdrawals' AND column_name='idempotency_key') AS is_present
UNION ALL
SELECT 'column: created_by', EXISTS(SELECT 1 FROM information_schema.columns WHERE table_name='withdrawals' AND column_name='created_by')
UNION ALL
SELECT 'column: updated_at', EXISTS(SELECT 1 FROM information_schema.columns WHERE table_name='withdrawals' AND column_name='updated_at')
UNION ALL
SELECT 'index: idx_withdrawals_idempotency_key', EXISTS(SELECT 1 FROM pg_indexes WHERE tablename='withdrawals' AND indexname='idx_withdrawals_idempotency_key')
UNION ALL
SELECT 'function: financial_lock_key', EXISTS(SELECT 1 FROM pg_proc WHERE proname='financial_lock_key')
UNION ALL
SELECT 'function: calculate_available_withdrawal_equity_sql', EXISTS(SELECT 1 FROM pg_proc WHERE proname='calculate_available_withdrawal_equity_sql')
UNION ALL
SELECT 'function: create_withdrawal_atomic', EXISTS(SELECT 1 FROM pg_proc WHERE proname='create_withdrawal_atomic')
UNION ALL
SELECT 'function: update_withdrawal_atomic', EXISTS(SELECT 1 FROM pg_proc WHERE proname='update_withdrawal_atomic');
```

---

## Section B: Before Fingerprint & Economic Digest

Run this query immediately before DDL execution to compute the exact immutable before-state fingerprint:

```sql
-- ============================================================================
-- IMMUTABLE ECONOMIC FINGERPRINT (BEFORE STATE)
-- ============================================================================
SELECT 
  COUNT(*) AS total_rows,
  SUM(amount) AS total_amount_sum,
  COUNT(CASE WHEN effective_accounting_date IS NULL THEN 1 END) AS null_effective_dates,
  -- Deterministic cryptographic digest of all 9 core economic/immutable columns
  md5(string_agg(
    id::text || '|' || 
    COALESCE(investor_id, '') || '|' || 
    COALESCE(account_id, '') || '|' || 
    amount::text || '|' || 
    COALESCE(status, '') || '|' || 
    COALESCE(request_date::text, '') || '|' || 
    COALESCE(effective_accounting_date::text, '') || '|' || 
    COALESCE(year::text, '') || '|' || 
    COALESCE(month_number::text, ''),
    ',' ORDER BY id::text
  )) AS immutable_economic_fingerprint
FROM withdrawals;
```

---

## Section C: Exact Certified Migration Execution (DDL)

> **AUTHORIZATION REQUIRED:** Paste and execute the exact SQL below into the Supabase SQL Editor (`julhldzkiqdeuuoqmvlo`):
...
## Section D: Post-Install Schema & Function Verification

Execute this block immediately after migration installation to verify object creation, ownership, and role security:

```sql
-- ============================================================================
-- VERIFY 1: COLUMNS & INDEX
-- ============================================================================
SELECT 
  column_name, data_type, is_nullable
FROM information_schema.columns 
WHERE table_name = 'withdrawals' 
  AND column_name IN ('idempotency_key', 'created_by', 'updated_at', 'effective_accounting_date')
ORDER BY column_name;

SELECT indexname, indexdef 
FROM pg_indexes 
WHERE tablename = 'withdrawals' AND indexname = 'idx_withdrawals_idempotency_key';

-- ============================================================================
-- VERIFY 2: FUNCTIONS, SECURITY DEFINER & OWNER
-- ============================================================================
SELECT 
  p.proname,
...
WHERE p.proname IN (
  ...
  'update_withdrawal_atomic'
)
ORDER BY p.proname;

-- ============================================================================
-- VERIFY 3: EXECUTE PRIVILEGES ON SERVICE_ROLE & REVOKED PUBLIC
-- ============================================================================
SELECT 
  routine_name,
  grantee,
...
## Section E: Post-Install Data Integrity & Economic Digest Verification

Verify that total rows, amounts, statuses, and cryptographic digest are 100% identical to Section B:

```sql
-- ============================================================================
-- IMMUTABLE ECONOMIC FINGERPRINT (POST-INSTALL STATE)
-- MUST MATCH SECTION B FINGERPRINT EXACTLY (ZERO FINANCIAL DELTA)
-- ============================================================================
SELECT 
  COUNT(*) AS total_rows,
  SUM(amount) AS total_amount_sum,
  ...
  md5(string_agg(
    ...
    COALESCE(month_number::text, ''),
    ',' ORDER BY id::text
  )) AS post_install_economic_fingerprint
FROM withdrawals;
```

---

## Section F: Safe Read-Only Equity Evaluation Check (Kyle Landon Control)

Run a read-only equity calculation against Kyle Landon (`inv_835ffffd`), whose start date was cleanly reconciled:

```sql
-- ============================================================================
-- SAFE READ-ONLY FUNCTION TEST: KYLE LANDON CONTROL
-- Expected: Available equity = $75,000.00 at 2026-08-01 (Zero start/open conflict)
-- ============================================================================
SELECT 
  'inv_835ffffd' AS investor_id,
...
## Section G: Failure & Rollback Contingency Plan

### Case 1: Installation Fails During DDL Execution
If an error occurs before complete execution, PostgreSQL DDL statements inside the transaction abort automatically. If running statements individually:

```sql
-- Immediate cleanup if partial DDL failure occurs
DROP FUNCTION IF EXISTS update_withdrawal_atomic(UUID, NUMERIC, TEXT, TEXT, TEXT);
DROP FUNCTION IF EXISTS create_withdrawal_atomic(TEXT, TEXT, NUMERIC, DATE, TEXT, TEXT, TEXT, TEXT);
DROP FUNCTION IF EXISTS calculate_available_withdrawal_equity_sql(TEXT, TEXT, DATE, UUID);
DROP FUNCTION IF EXISTS financial_lock_key(TEXT);
DROP INDEX IF EXISTS idx_withdrawals_idempotency_key;
ALTER TABLE withdrawals 
...
### Case 2: Post-Installation Rollback (After New Transactions Exist)
Once new withdrawals have been created with `idempotency_key`, **do NOT drop columns**. Use forward remediation:
1. Preserve table columns to protect audit trail and idempotency history.
2. If function bugs are identified, replace functions via `CREATE OR REPLACE FUNCTION` rather than dropping columns.
