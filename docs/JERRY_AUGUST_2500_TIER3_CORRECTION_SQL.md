# Jerry's Rogue Jets — Tier 3 August $2,500 Withdrawal Atomic Correction SQL Package
...
**Financial Delta:** `$0.00` accounting residual (Eligible capital `$546,135.92` $\to$ `$543,635.92`)  
**Status:** **`DESIGN_ONLY / AWAITING_EXPLICIT_TIER3_EXECUTION_AUTHORIZATION`**

---

## Section A: Read-Only Preflight Inspection Query

Run this in the Supabase SQL Editor to verify the pre-state:

```sql
-- 1. Verify Investor & Account Baseline
SELECT 
  i.id AS investor_id,
  i.portal_username,
...
-- 2. Verify August History Row Pre-State
SELECT 
  id AS history_id,
  year,
  month_number,
...
-- 3. Verify Package B Available Equity (Must equal $546,135.92)
SELECT 
...
-- 4. Verify No Duplicate August 2026 Withdrawal
SELECT COUNT(*) AS active_august_withdrawals
FROM withdrawals
...
## Section B: Guarded Tier 3 Forward Transaction Script
...
## Section C: Guarded Tier 3 Atomic Reversal Script

If reversal is ever required, execute this atomic reversal script:
...
BEGIN
  ...
  -- 2. Atomic Reversal Mutation Pair
...
## Section D: Read-Only Post-Execution Verification Queries

```sql
-- 1. Verify New Withdrawal Source Record
SELECT 
  id,
  investor_id,
  account_id,
...
-- 2. Verify Aligned August History Row
-- Expected: withdrawals = 2500.00, ending_balance = $543,635.92
SELECT 
  year,
  month_number,
  month,
...
-- 3. Post-Correction Package B Available Equity Evaluation
