# Gary Larson — Scheduled September $120k Deposit Void Tier 3 Correction SQL
...
**Classification:** **`DEPOSIT_VOID_ONLY_CORRECTION`**  
**Authorized Scope:** Live preflight proves Gary's start date (`2026-08-01`), account open date (`2026-08-01`), starting capital (`$487,000.00`), and August history (`$487,000.00`) are **ALREADY CORRECT** in production. This correction strictly applies the auditable non-economic void treatment to scheduled September deposit `dep_94a0ffe1` ($120,000.00) to prevent double-counting.
...
SELECT 
  ...
  (SELECT date FROM deposits WHERE id = 'dep_94a0ffe1') AS dep_94a0ffe1_date,
...
BEGIN
  ...
  -- 2. CAS ASSERTIONS (VERIFY ALREADY-CORRECT METADATA)
...
```sql
SELECT 
  -- 1. Preserved Correct Account Baseline
  (
