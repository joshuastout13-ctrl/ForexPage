 ...
 * 
 * Semantics:
 * Total Deposits = Initial funded cash amount + subsequent valid external deposits.
 * Excludes: investment gains, commission earnings, transfers, VOID / CANCELLED records.
...
export function calculateTotalDeposits({
  ...
  let total = new Decimal(baselineCashIn || startingCapital || 0);

  (depositRows || []).forEach(d => {
    const typeStr = String(d.type || '').toUpperCase();
    const statusStr = String(d.status || '').toUpperCase();
    if (typeStr === 'VOID' || statusStr === 'VOID' || statusStr === 'CANCELLED' || typeStr === 'COMMISSION') {
      return;
    }
