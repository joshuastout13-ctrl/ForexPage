 ...
 *
 * Authoritative Client Contract (Josh Aug 2026):
 * "This box was supposed to show the total performance and the percentage that the entire account has earned.
 * Meaning if I put in $1 million and now the account has $3 million, it should show a 200% gain."
 *
 * Economic Formulas:
 * 1. Contributed Invested Capital = Initial / Cutover Baseline Contributed Capital + Cumulative External Deposits
 * 2. Lifetime Total Economic Gain $ = (Current Balance + Cumulative Completed Withdrawals) - Contributed Invested Capital
 * 3. Lifetime Total Performance % = Contributed Invested Capital > 0 ? (Lifetime Total Economic Gain $ / Contributed Invested Capital) * 100 : 0
 *
 * Uses Decimal.js for precise cent math.
 */
export function calculateLifetimePerformance({
  ...
  const baseCapital = (cutoverBaseline !== null && cutoverBaseline !== undefined && !isNaN(Number(cutoverBaseline)))
    ? new Decimal(cutoverBaseline)
    : new Decimal(startingCapital || 0);

  const decDeposits = new Decimal(cumulativeDeposits || 0);
  const decWithdrawals = new Decimal(cumulativeWithdrawals || 0);
  const decBalance = new Decimal(currentBalance || 0);

  const contributedBasis = baseCapital.add(decDeposits);
  const totalValueGenerated = decBalance.add(decWithdrawals);
  const totalPerformanceDollar = totalValueGenerated.sub(contributedBasis);

  const totalPerformancePct = contributedBasis.gt(0)
    ? totalPerformanceDollar.div(contributedBasis).mul(100)
    : new Decimal(0);

  return {
    contributedBasis: contributedBasis.toNumber(),
