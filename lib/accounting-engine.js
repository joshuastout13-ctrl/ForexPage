...
export function calculateBalanceAffectingDeposits({
  ...
  (depositRows || []).forEach(d => {
    ...
    const treatment = String(d.accounting_treatment || 'NEW_CASH').toUpperCase();
...
export function calculateLifetimePerformance({
  ...
  if (totalExternalCashSent !== null && totalExternalCashSent !== undefined) {
    ...
    const isProven = hasConfirmedExternalCashRecords && decCash.gt(0);

    if (!isProven || decCash.lte(0)) {
    ...
    return {
      ...
      isProven: true,
      provenanceStatus: provenanceOriginType || 'EXTERNAL_CASH_CONFIRMED'
    };
  }
  ...
  return {
    ...
    totalPerformanceDollar: isProven ? totalPerformanceDollar.toNumber() : null,
    totalPerformancePct: isProven && totalPerformancePct !== null ? totalPerformancePct.toNumber() : null,
    isProven,
    provenanceStatus
  };
}
