 ...
 * 1. Total External Cash Sent = Initial Proven Cash Funding + Qualifying Additional External Cash Deposits
 * 2. Total Performance $ = Current Balance - Total External Cash Sent
 * 3. Total Performance % = Total External Cash Sent > 0 ? ((Current Balance - Total External Cash Sent) / Total External Cash Sent) * 100 : 0
 *
 * IMPORTANT: Withdrawals are NOT added back (Current Balance naturally reflects withdrawals).
 *
 * Uses Decimal.js for precise cent math.
 */
export function calculateLifetimePerformance({
  ...
  let isProven = false;
  let provenanceStatus = 'UNKNOWN_REQUIRES_REVIEW';

  if (initialCashContribution !== null && initialCashContribution !== undefined && !isNaN(Number(initialCashContribution))) {
    baseCapital = new Decimal(initialCashContribution);
    isProven = true;
    provenanceStatus = provenanceOriginType || 'EXTERNAL_CASH';
  } else if (cutoverBaseline !== null && cutoverBaseline !== undefined && !isNaN(Number(cutoverBaseline))) {
    baseCapital = new Decimal(cutoverBaseline);
    isProven = true;
    provenanceStatus = 'CUTOVER_BASELINE';
  } else if (startingCapital > 0) {
    // Unverified starting capital fallback - strictly marked UNPROVEN
    baseCapital = new Decimal(startingCapital);
    isProven = false;
    provenanceStatus = 'UNPROVEN_STARTING_CAPITAL';
  }

  const decDeposits = new Decimal(cumulativeDeposits || 0);
  const decBalance = new Decimal(currentBalance || 0);

  const totalExternalCashSent = baseCapital.add(decDeposits);
  const totalPerformanceDollar = decBalance.sub(totalExternalCashSent);

  const totalPerformancePct = totalExternalCashSent.gt(0)
    ? totalPerformanceDollar.div(totalExternalCashSent).mul(100)
    : new Decimal(0);

  return {
    contributedBasis: totalExternalCashSent.toNumber(),
    totalExternalCashSent: totalExternalCashSent.toNumber(),
