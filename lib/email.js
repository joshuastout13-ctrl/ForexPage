...
/**
 * Sends single or batch email via Resend API.
 * 
 * @param {Object} opts
 ...
 */
export async function sendEmail({ recipients, subject, body, from, replyTo }) {
  if (!resend) {
  ...
  // Resend supports sending to up to 100 recipients per batch call or using Bcc/To
  // For individual delivery so recipients don't see each other's email address, we send individually or in batch payload array
  const results = [];
  const errors = [];

  // Send in batches of 50 to maintain clean rate limit handling
  const BATCH_SIZE = 50;
  for (let i = 0; i < recipients.length; i += BATCH_SIZE) {
    const batch = recipients.slice(i, i + BATCH_SIZE);
    
    // Build batch payload array for Resend batch endpoint
