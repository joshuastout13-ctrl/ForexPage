...
export const HISTORICAL_COMPARISON_TOLERANCE = 25.00;
...
export function runFullHistoricalAudit({
  ...
  let catReconciled = 0;
  let catCentMatch = 0;
  let catAcceptableHistoricalDiff = 0;
  ...
  monthsToAudit.forEach(({ year, month }) => {
    ...
    periodRun.investors.forEach(invCalc => {
      ...
      if (isPreStartMonth) {
        ...
        if (username === 'aray' && diff > 0) {
          classification = "LEGACY_ROUNDING";
          clientDescription = `Historical $${diff.toFixed(2)} cent rounding difference (Ashlee Ray July checkpoint). Preserved as stored. Non-blocking.`;
          catLegacyRounding++;
          mCentMatch++;
          stats.centMatch++;
        } else {
          classification = "CENT_MATCH";
          clientDescription = `Accepted cent-level match ($${diff.toFixed(2)} <= $0.01 tolerance). Reconciled.`;
          catCentMatch++;
          mCentMatch++;
          stats.centMatch++;
        }
      } else if (diff <= HISTORICAL_COMPARISON_TOLERANCE) {
        // Authoritative Client Tolerance ($25.00 or less)
        classification = "ACCEPTABLE_HISTORICAL_DIFFERENCE";
        clientDescription = `Acceptable historical difference ($${diff.toFixed(2)} <= $25.00 comparison tolerance). Reconciled for audit review.`;
        catAcceptableHistoricalDiff++;
        mAcceptableDiff++;
        stats.acceptableDiff = (stats.acceptableDiff || 0) + 1;
      } else if (year < 2026 || (year === 2026 && month <= 7)) {
        // All historical differences > $25 prior to August 2026 automation cutover
        classification = "LEGACY_MANUAL";
        clientDescription = `Documented historical legacy baseline difference ($${diff.toFixed(2)} > $25.00 tolerance). Requires historical review.`;
        catLegacyManual++;
        mLegacyManual++;
        stats.legacy++;
        stats.overallStatus = "REQUIRES_REVIEW";
    ...
    monthlySummaries.push({
      ...
      acceptableDiffCount: mAcceptableDiff,
  ...
  const sumCategories = catReconciled + catCentMatch + catAcceptableHistoricalDiff + catLegacyManual + catLegacyRounding + catLegacyLedgerTiming + catBadSourceData + catRuleConfiguration + catPreStart + catEngineBug + catUnknown + catOther;
  ...
  const bbeckControlPass = bbeckDetails.length > 0 && bbeckDetails.every(d => d.classification === "RECONCILED" || d.classification === "CENT_MATCH" || d.classification === "ACCEPTABLE_HISTORICAL_DIFFERENCE" || d.classification === "LEGACY_ROUNDING" || d.classification === "LEGACY_MANUAL" || d.classification === "PRE_START");
  ...
  const flaggedItemsCount = investorMonthDetails.filter(d => d.classification === "LEGACY_MANUAL" || d.isBlocking).length;
  const flaggedAccountsCount = Object.values(investorStatsMap).filter(s => s.legacy > 0 || s.blocking > 0).length;
  ...
  return {
    ...
    auditMetadata: {
      ...
      historicalComparisonTolerance: HISTORICAL_COMPARISON_TOLERANCE,
      explanatoryNote: "Per fund administrator guidance, historical stored-vs-calculated differences of $25.00 or less are treated as acceptable for audit review. Accounting calculations themselves remain cent-accurate."
    },
    summaryCards: {
      ...
      acceptableHistoricalDifferences: catAcceptableHistoricalDiff,
      historicalManualDifferences: catLegacyManual,
      totalHistoricalLegacyDifferences: catCentMatch + catLegacyRounding + catAcceptableHistoricalDiff + catLegacyManual,
      flaggedItemsCount,
      ...
      acceptableDiff: catAcceptableHistoricalDiff,
