...
export function calculateAccountingPeriod({
  ...
  const investorMap = {};
  (investors || []).forEach(i => {
    if (i.id) investorMap[String(i.id).toLowerCase()] = i;
    if (i.portal_username) investorMap[String(i.portal_username).toLowerCase()] = i;
    if (i.portalusername) investorMap[String(i.portalusername).toLowerCase()] = i;
  });
  ...
  for (const inv of activeInvestors) {
    ...
    const enrichedShares = applicableShares.map(s => {
      const recKey = String(s.recipient_investor_id || s.recipient_id || s.recipientId || '').toLowerCase();
      const recInv = investorMap[recKey];
      const recName = recInv ? (`${recInv.first_name || ''} ${recInv.last_name || ''}`.trim() || recInv.portal_username || recInv.id) : (s.recipient_name || s.recipientName || "Recipient");
      const recUser = recInv ? (recInv.portal_username || recInv.id) : (s.recipient_username || s.recipientUsername || s.recipient_investor_id || s.recipient_id || "unknown");
      return {
        ...s,
        recipientId: s.recipient_investor_id || s.recipient_id || s.recipientId,
    ...
    const result = calculateInvestorMonth({
      ...
      commissionShares: enrichedShares,
      recurringDraw
    });
    ...
    if (nonFirstDaysForInv.length > 0) flags.push("CASHFLOW_EFFECTIVE_DATE_REVIEW");
