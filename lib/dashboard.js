...
export async function buildInvestorDashboard(investorId, preloadedData = null) {
  ...
  const decSplitMultiplier = new Decimal(splitPct).div(100);
  ...
  const todayNetDollar = new Decimal(liveBase).mul(todayGrossPct).div(100).mul(decSplitMultiplier).toNumber();
  const weekNetDollar = new Decimal(liveBase).mul(weekGrossPct).div(100).mul(decSplitMultiplier).toNumber();
  const monthNetDollar = new Decimal(liveBase).mul(monthGrossPct).div(100).mul(decSplitMultiplier).toNumber();
  const lastMonthNetDollar = lastBreakdownRow ? lastBreakdownRow.gain : new Decimal(liveBase).mul(lastMonthGrossPct).div(100).mul(decSplitMultiplier).toNumber();
  const yearNetDollar = totalGain.toNumber();
  ...
  const accountPerformance = {
    splitPct,
    month: { netDollar: monthNetDollar, netReturnPct: monthNetPct, status: "PROJECTED / LIVE" },
    lastMonth: { netDollar: lastMonthNetDollar, netReturnPct: lastMonthNetPct, status: "FINALIZED" },
    year: { netDollar: yearNetDollar, status: "CUMULATIVE YTD" }
  };

  // 7. Overall Performance & Cumulative Contributed Cash (Total Deposits)
  // Total Deposits = cumulative external cash contributed (initial baseline cash + subsequent valid deposits)
        ...
        if (isPastOrCurrent) {
          ...
          if (!grouped[sourceId]) {
            ...
            sourceAccs.forEach(acc => {
              ...
              if (srcHist && num(srcHist.ending_balance) > 0) {
