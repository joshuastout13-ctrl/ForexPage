  ...
  if (myEarnings.length > 0) {
    myEarnings.forEach(e => {
      ...
      if (eYear === targetYear) {
        ...
        if (!grouped[sourceId]) {
          const sourceInvestor = rawInvestors.find(i => String(i.id || i.investorid || '').toLowerCase() === String(sourceId).toLowerCase()) || {};
          ...
          const share = unifiedSharesTable.find(r => 
            r.recipient_investor_id.toLowerCase() === internalId.toLowerCase() && 
            r.source_investor_id.toLowerCase() === String(sourceId).toLowerCase() &&
    ...
    const myShares = unifiedSharesTable.filter(s => s.recipient_investor_id.toLowerCase() === internalId.toLowerCase() && s.status !== 'cancelled' && s.status !== 'ended');
    ...
    myShares.forEach(share => {
      ...
      const sourceAccounts = accounts.filter(a => String(a.investor_id || a.id || '').toLowerCase() === sourceId.toLowerCase());
