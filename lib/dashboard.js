...
import { calculateBalanceAffectingDeposits, calculateTotalExternalCash, calculateLifetimePerformance, calculateLifetimeSettledWithdrawals } from "./accounting-engine.js";
import { getFundAccountingDate, evaluateMonthState, getLastCompletedMonth, MonthState } from "./month-state.js";
...
export async function buildInvestorDashboard(investorId, preloadedData = null, options = {}) {
  ...
  if (preloadedData) {
    ...
    // 1. Fetch core/small tables first
    const [invs, accs, rets, commS, commR, liveData, cuts] = await Promise.all([
    ...
    const investor = investors.find((r) => {
      const rowUser = String(r.portal_username ?? r.portalusername ?? r.username ?? "").trim().toLowerCase();
      const rowId = String(r.investorsinvestorid ?? r.investorid ?? r.id ?? "").trim().toLowerCase();
      return rowUser === targetId || rowId === targetId;
    });

    if (!investor) throw Object.assign(new Error(`Investor not found: "${id}"`), { status: 404 });

    const internalId = String(investor.investorsinvestorid ?? investor.investorid ?? investor.id ?? investor.portalusername ?? "").trim();

    const investorIdSet = new Set([
      internalId, investor.id, investor.portalusername, investor.portal_username, investor.username,
      investor.email, investor.portal_email, investor.investorsinvestorid, investor.investorid
    ].filter(Boolean).map(s => String(s).trim().toLowerCase()));

    // Resolve source investor IDs needed for commission calculations
    const requiredInvestorIds = new Set(investorIdSet);
    [...commS, ...commR].forEach(r => {
      const recId = String(r.recipient_investor_id || r.recipient_id || r.recipientid || '').trim().toLowerCase();
      if (investorIdSet.has(recId)) {
        const srcId = String(r.source_investor_id || r.investor_id || r.investorid || '').trim().toLowerCase();
        if (srcId) requiredInvestorIds.add(srcId);
      }
    });

    const expandedRequiredIds = new Set(requiredInvestorIds);
    rawInvestors.forEach(inv => {
       const mappedIds = [inv.id, inv.portal_username, inv.investorsinvestorid, inv.investorid].filter(Boolean).map(s => String(s).trim().toLowerCase());
       if (mappedIds.some(m => requiredInvestorIds.has(m))) {
           mappedIds.forEach(m => expandedRequiredIds.add(m));
       }
    });
    const idArray = Array.from(expandedRequiredIds);

    // 2. Fetch large transaction tables filtered specifically for required investors
    const byInvestorId = q => q.in("investor_id", idArray);
    const byRecipientId = q => q.in("recipient_id", idArray);

    const [deps, wds, hist, commE] = await Promise.all([
      readSupabaseTable("deposits", { queryModifier: byInvestorId }),
      readSupabaseTable("withdrawals", { queryModifier: byInvestorId }),
      readSupabaseTable("investor_monthly_history", { queryModifier: byInvestorId }),
      readSupabaseTable("commission_earnings", { queryModifier: byRecipientId })
    ]);
  ...
  const investorAccounts = accounts.filter((r) => {
    const rowInvId = String(r.investor_id ?? r.investorid ?? r.id ?? "").trim().toLowerCase();
    return rowInvId === internalId.toLowerCase() || (investorIdSet && investorIdSet.has(rowInvId));
  });
  let startCapital = 0;
  ...
  const splitPct = num(investor.split_pct ?? investor.investorsplit ?? investor.investorsplitpct ?? investor.split, 100);
  const recurringDraw = num(investor.recurringmonthlydraw ?? investor.monthlydraw, 0);
  const startDateStr = String(investor.start_date ?? investor.opendate ?? investor.startdate ?? investor.date ?? "");
  ...
  const monthlyReturns = (returnsSheet || []).filter((r) => num(r.Year ?? r.year) === targetYear);
  const monthlyHistory = monthlyReturns
    .map((r) => ({
  ...
  depositsSheet.forEach((r) => {
    const rowInvId = String(r.investor_id ?? r.investorid ?? r.id ?? "").trim().toLowerCase();
    if (rowInvId !== internalId.toLowerCase() && !investorIdSet.has(rowInvId)) return;

    // Exclude VOID and CANCELLED records (both legacy type='VOID' and new status column)
    const typeStr = String(r.type ?? '').toUpperCase();
    const statusStr = String(r.status ?? '').toUpperCase();
    if (typeStr === 'VOID' || statusStr === 'VOID' || statusStr === 'CANCELLED') return;

    // CRITICAL: HISTORICAL_PROVENANCE deposits do NOT affect the accounting balance.
    // They establish Total Deposits provenance only. The economic effect of this cash
    // is already present in the imported/cutover accounting baseline.
    const treatment = String(r.accounting_treatment || 'NEW_CASH').toUpperCase();
    const isHistoricalProvenance = treatment === 'HISTORICAL_PROVENANCE';

    const m = monthNum(r);
    if (m >= 1 && m <= 12) {
      const amt = num(r.amount ?? r.Amount, 0);
      // Only NEW_CASH deposits feed the monthly balance compounding loop
      if (!isHistoricalProvenance && !r.is_commission && !r.iscommission) {
  ...
  const effectiveAsOfDate = options?.asOfDate || preloadedData?.asOfDate || null;
  const fundDate = getFundAccountingDate(effectiveAsOfDate);
  const currentMonthIdx = fundDate.monthNumber; // 1-12
  const currentYearIdx = fundDate.year;
  ...
  for (const row of monthlyHistory) {
    ...
    // Check for authorized cutover adjustment for this investor and period
    const cutoverRow = (cutoverTable || []).find(c => {
      const cInvId = String(c.investor_id || c.investorid || '').trim().toLowerCase();
      return (cInvId === internalId.toLowerCase() || (investorIdSet && investorIdSet.has(cInvId))) &&
        num(c.year) === targetYear &&
        num(c.month_number || c.month) === m;
    });
    
    // Calculation order per Josh Confirmed General Accounting Rule:
    // 1. prior ending balance + prior capitalized commissions (or authorized cutover baseline)
    // 2. + new deposits
    // 3. - approved/completed withdrawals
    // 4. apply gross return x split % on return-eligible capital base
    // 5. - recurring draw
    const startingBal = cutoverRow
      ? new Decimal(cutoverRow.authorized_opening_balance)
      : (historyRow ? new Decimal(historyRow.opening_balance || 0) : balance);

    const effectiveDeps = historyRow && historyRow.deposits !== null && historyRow.deposits !== undefined
      ? new Decimal(historyRow.deposits)
      : deps;
    const effectiveWds = historyRow && historyRow.withdrawals !== null && historyRow.withdrawals !== undefined
      ? new Decimal(historyRow.withdrawals)
      : wds;

    const adjustedStart = startingBal.add(effectiveDeps).sub(effectiveWds);
    ...
    const isPastOrCurrent = (monthState !== MonthState.FUTURE);
  ...
  const myWithdrawals = (withdrawalsSheet || []).filter(r => {
    const rId = String(r.investorid ?? r.investor_id ?? r.id ?? "").trim().toLowerCase();
    const rAccId = String(r.account_id ?? r.accountid ?? "").trim().toLowerCase();
    return myIdSet.has(rId) || (rAccId && myIdSet.has(rAccId));
  });

  // Authoritative Lifetime Settled Withdrawals (Josh confirmed Sep 2026):
  // Scope starts at account activation, includes qualifying withdrawals across ALL calendar years,
  // includes current open month immediately once settled (Completed or Approved), excludes pending/cancelled.
  const lifetimeWithdrawalsResult = calculateLifetimeSettledWithdrawals({
    withdrawalRows: myWithdrawals,
    accountStartDate: startDate,
    asOfDate: effectiveAsOfDate
  });

  // Authoritative Total External Cash Sent (includes HISTORICAL_PROVENANCE + NEW_CASH).
  // This is the denominator for Total Performance % and the Total Deposits display value.
  // It does NOT include starting_capital, commissions, VOID, or CANCELLED records.
  const totalExternalCashResult = calculateTotalExternalCash({
    depositRows: myDeposits,
    accountStartDate: startDate
    // No targetYear/maxMonth filter — lifetime total, all confirmed records
  });

  // YTD balance-affecting deposits (NEW_CASH only, current year, up to current month)
  // Used as the effectiveCashIn/externalDepositsYtd display value.
  const effectiveCashIn = calculateBalanceAffectingDeposits({
    depositRows: myDeposits,
    targetYear,
  ...
  // 3-State Provenance Completeness Determination:
  // - COMPLETE: explicitly certified by admin on the account(s).
  // - PARTIAL: at least one confirmed external cash deposit exists, but funding completeness is NOT certified.
  // - UNKNOWN: no confirmed external cash records exist.
  const targetAccounts = activeAccounts.length > 0 ? activeAccounts : investorAccounts;
  const isExplicitlyComplete = targetAccounts.length > 0 && targetAccounts.every(a => {
    const s = String(a.external_cash_provenance_status || a.externalcashprovenancestatus || '').toUpperCase();
    return s === 'COMPLETE';
  });

  let provenanceCompletenessStatus = 'UNKNOWN';
  if (isExplicitlyComplete && totalExternalCashResult.hasConfirmedRecords) {
    provenanceCompletenessStatus = 'COMPLETE';
  } else if (totalExternalCashResult.hasConfirmedRecords && totalExternalCashResult.total > 0) {
  ...
  const provenanceCertifiedAt = primaryAccount.provenance_certified_at || primaryAccount.provenancecertifiedat || null;
  const provenanceCertifiedBy = primaryAccount.provenance_certified_by || primaryAccount.provenancecertifiedby || null;
  const provenanceCertificationNotes = primaryAccount.provenance_certification_notes || primaryAccount.provenancecertificationnotes || null;

  // Build the lifetime performance calculation using the authoritative path.
  // FAIL-CLOSED: Total Performance is ONLY calculated if provenanceCompletenessStatus is COMPLETE.
  const lifetimePerf = calculateLifetimePerformance({
    totalExternalCashSent: totalExternalCashResult.total,
    hasConfirmedExternalCashRecords: totalExternalCashResult.hasConfirmedRecords,
  ...
  const lastCompletedMonth = getLastCompletedMonth(targetYear, effectiveAsOfDate);
  ...
  return {
    ...
    summary: {
      ...
      totalWithdrawals: lifetimeWithdrawalsResult.total,
      total_withdrawals: lifetimeWithdrawalsResult.total,
      totalWithdrawalsCount: lifetimeWithdrawalsResult.count,
      ytdWithdrawals: totalWithdrawals.toNumber(),
      totalCashIn: effectiveCashIn,
      externalDepositsYtd: effectiveCashIn,
      // totalExternalCashSent: lifetime confirmed external cash (NEW_CASH + HISTORICAL_PROVENANCE)
      // This is authoritative Total Deposits shown on the investor dashboard.
      totalExternalCashSent: totalExternalCashResult.total,
      externalDepositsLifetime: totalExternalCashResult.total,
      contributedBasis,
      lifetimeContributedBasis: contributedBasis,
      // totalPerformancePct / totalPerformanceDollar are null when no confirmed external cash exists.
      // Callers must display 'Not Established' — never fabricate 0%.
      totalPerformancePct,
      totalPerformanceDollar,
      isProven: lifetimePerf.isProven,
      provenanceStatus: lifetimePerf.provenanceStatus,
      provenanceCompletenessStatus,
      provenanceCertifiedAt,
      provenanceCertifiedBy,
      provenanceCertificationNotes,
      hasConfirmedExternalCash: totalExternalCashResult.hasConfirmedRecords,
