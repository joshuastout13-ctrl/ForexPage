...
export async function buildInvestorDashboard(investorId) {
  ...
  if (useSupabase) {
    ...
    [rawInvestors, accounts, returnsSheet, depositsSheet, withdrawalsSheet, historyTable, live] = await Promise.all([
  ...
  for (const row of monthlyHistory) {
    ...
    let adjustedStart = Math.max(0, balance + deps - wds);
    let gain = 0;
    let ending = 0;
    ...
    if (historyRow) {
      ...
      adjustedStart = num(historyRow.opening_balance) + num(historyRow.deposits) - num(historyRow.withdrawals);
      gain = num(historyRow.manual_gain_amount ?? (adjustedStart * (num(historyRow.manual_return_pct ?? effPct) / 100)));
      ending = num(historyRow.ending_balance);
      isManual = bool(historyRow.is_manual ?? historyRow.ismanual);
    } else {
      // Standard calculation
      gain = adjustedStart * (effPct / 100);
      ending = Math.max(0, adjustedStart + gain - recurringDraw);
    }
    ...
    if (isHistoricalOrCurrent) {
      totalGain += gain;
      totalWithdrawals += (wds + (ending > 0 || recurringDraw > 0 ? recurringDraw : 0));
  ...
  // Override live performance year with investor-specific metric
  live.year = fmt(investorYtdPct);
