...
export async function buildInvestorDashboard(investorId, preloadedData = null) {
  ...
  // Authoritative Lifetime Entire-Account Performance Contract (Josh Aug 2026):
  // Lifetime Total Performance $ = (Current Balance + Total Completed Withdrawals) - Contributed Invested Capital
  // Lifetime Total Performance % = (Lifetime Total Performance $ / Contributed Invested Capital) * 100
  const lifetimePerf = calculateLifetimePerformance({
    startingCapital: startCapital,
    cutoverBaseline,
    cumulativeDeposits: lifetimeDeposits,
    cumulativeWithdrawals: totalWithdrawals.toNumber(),
