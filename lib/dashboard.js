...
export async function buildInvestorDashboard(investorId, preloadedData = null) {
  ...
  if (preloadedData) {
    ({ rawInvestors, accounts, returnsSheet, depositsSheet, withdrawalsSheet, historyTable, commissionEarningsTable, commissionSharesTable, live } = preloadedData);
    if (useSupabase) {
    ...
    [rawInvestors, accounts, returnsSheet, depositsSheet, withdrawalsSheet, historyTable, commissionEarningsTable, commissionSharesTable, live] = await Promise.all([
  ...
  for (const row of monthlyHistory) {
    ...
    const pendingWds = pendingWdByMonth[m] || 0;

    // 1. Gain Basis (Strictly Opening Balance)
    let gainBasis = balance;
    let gain = 0;
    let ending = 0;
    ...
    // Adjusted Start is the Opening Balance + Deposits - Withdrawals
    ...
    if (historyRow) {
      ...
      const manualGainBasis = manualStart;
      ...
      ending = Math.max(0, balance + gain + deps - wds - recurringDraw);
    }
    ...
    balance = ending + commissionsEarned;
  }
  ...
  live.lastMonth = lastMonthPctNum.toFixed(2) + "%";
  ...
  const displayCommMonthIdx = latestGlobalCommMonth || currentMonthIdx;
  ...
  if (commissionEarningsTable && commissionEarningsTable.length > 0) {
    ...
    myEarnings.forEach(e => {
      ...
      if (!grouped[sourceId]) {
        ...
        // Find commission share (percent) based on the latest active share
        let percent = 0;
        if (commissionSharesTable && commissionSharesTable.length > 0) {
           const share = commissionSharesTable.find(r => 
             String(r.recipient_investor_id).toLowerCase() === internalId.toLowerCase() && 
             (String(r.source_investor_id).toLowerCase() === String(sourceId).toLowerCase()) &&
             (!r.effective_end_date)
           );
           if (share) percent = num(share.commission_percent);
        }
      ...
      if (num(e.month_number) === displayCommMonthIdx) {
  ...
  if (manualYearAmt > 0) {
    grouped["manual_deposit"] = {
      ...
      percent: "-", // Manual deposits don't have a fixed percentage
