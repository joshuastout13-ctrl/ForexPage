...
export async function buildInvestorDashboard(investorId, preloadedData = null) {
  ...
  const liveBase = summaryBalance.toNumber();
  ...
  // Augment live object with lastMonth performance from monthly returns history
  const lastMonthNum = currentMonthIdx === 1 ? 12 : currentMonthIdx - 1;
  let lastMonthRow = monthlyHistory.find(r => r.monthNumber === lastMonthNum);
  if (!lastMonthRow || Number(lastMonthRow.grossReturnPct || 0) === 0) {
    const prevCompleted = monthlyHistory
      .filter(r => r.monthNumber < currentMonthIdx && Number(r.grossReturnPct || 0) !== 0)
      .sort((a, b) => b.monthNumber - a.monthNumber);
    if (prevCompleted.length > 0) {
      lastMonthRow = prevCompleted[0];
    }
  }
  const lastMonthPctNum = lastMonthRow ? Number(lastMonthRow.grossReturnPct || 0) : 0;
  live.lastMonth = `${lastMonthPctNum >= 0 ? "+" : ""}${lastMonthPctNum.toFixed(2)}%`;

  // Fallback: If live.month is 0.00% while live.week is active (>0), use live.week as month fallback
  const weekPctVal = pctToNum(live.week);
  let monthPctVal = pctToNum(live.month);
  if (monthPctVal === 0 && weekPctVal > 0) {
    monthPctVal = weekPctVal;
    live.month = `${monthPctVal >= 0 ? "+" : ""}${monthPctVal.toFixed(2)}%`;
  }

  const liveDollarGains = {
    today: liveBase * (pctToNum(live.today) / 100),
    week: liveBase * (weekPctVal / 100),
    month: liveBase * (monthPctVal / 100),
    lastMonth: liveBase * (lastMonthPctNum / 100),
    year: totalGain.toNumber()
  };
  ...
  const totalPerformancePct = effectiveCashIn > 0 ? ((summaryBalance.toNumber() - effectiveCashIn) / effectiveCashIn) * 100 : 0;
  const totalPerformanceDollar = summaryBalance.toNumber() - effectiveCashIn;
  ...
  let latestGlobalCommMonth = 0;
  if (commissionEarningsTable && commissionEarningsTable.length > 0) {
    const thisYearEarnings = commissionEarningsTable.filter(r => num(r.year) === targetYear);
    if (thisYearEarnings.length > 0) {
      latestGlobalCommMonth = Math.max(...thisYearEarnings.map(e => num(e.month_number)));
    }
  }
  const manualCommMonths = Object.keys(commDepByMonth).map(Number);
  if (manualCommMonths.length > 0) {
    latestGlobalCommMonth = Math.max(latestGlobalCommMonth, ...manualCommMonths);
  }

  let displayCommMonthIdx = currentMonthIdx;
  if (targetYear < currentYearIdx) {
    displayCommMonthIdx = 12;
  } else if (latestGlobalCommMonth > 0) {
    displayCommMonthIdx = Math.min(latestGlobalCommMonth, currentMonthIdx);
  }
  ...
  if (myEarnings.length > 0) {
    myEarnings.forEach(e => {
      ...
      if (eYear === targetYear) {
        ...
        if (!grouped[sourceId]) {
          ...
          let percent = 0;
          const share = unifiedSharesTable.find(r => 
            investorIdSet.has(r.recipient_investor_id.toLowerCase()) && 
            sourceIdSet.has(r.source_investor_id.toLowerCase()) &&
    ...
    // Dynamic Fallback: calculate from unifiedSharesTable
    const myShares = unifiedSharesTable.filter(s => investorIdSet.has(s.recipient_investor_id.toLowerCase()) && s.status !== 'cancelled' && s.status !== 'ended');
