...
export async function buildInvestorDashboard(investorId) {
  ...
  let totalGain = 0;
  let totalWithdrawals = 0; // Sum of recurring draws + one-time withdrawals
  const breakdown = [];
  ...
  for (const row of monthlyHistory) {
    ...
    breakdown.push({
      ...
      startingBalance: balance, // Display original starting balance
  ...
  // 6. Live Performance Dollar Gains
  const liveBase = balance;
