...
export async function buildInvestorDashboard(investorId, preloadedData = null) {
  ...
  depositsSheet.forEach((r) => {
    ...
    if (m >= 1 && m <= 12) {
      ...
      if (r.is_commission || r.iscommission) {
        commDepByMonth[m] = (commDepByMonth[m] || 0) + amt;
      }
    }
  });
  ...
  for (const row of monthlyHistory) {
    ...
    // 1. Adjusted Basis for Gain (Balance + Deposits - Withdrawals)
    // We calculate gain AFTER withdrawals are subtracted to match user expectation (Gain / Starting Balance = %)
    let gainBasis = Math.max(0, balance + deps - wds);
    let gain = 0;
    let ending = 0;
    ...
    if (historyRow) {
      ...
      const manualGainBasis = Math.max(0, manualStart + manualDeps - manualWds);
      ...
      ending = Math.max(0, gainBasis + gain - recurringDraw);
    }
