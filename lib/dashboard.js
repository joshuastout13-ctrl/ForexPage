...
export async function buildInvestorDashboard(investorId) {
  ...
  if (!investor) {
    const dataSource = useSupabase ? "Supabase" : "Google Sheets";
    const availableIds = investors.slice(0, 5).map(inv => inv.portalusername || inv.id).join(", ");
    throw Object.assign(
      new Error(`Investor not found: "${targetId}" in ${dataSource}. Available: ${availableIds}...`), 
      { status: 404 }
    );
  }
