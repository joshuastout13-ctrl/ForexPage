...
export function extractSheetId(value) {
  ...
  // Standard Google Sheets URL pattern: /spreadsheets/d/([a-zA-Z0-9-_]+)
  const match = str.match(/\/spreadsheets\/d\/([a-zA-Z0-9-_]+)/);
  if (match && match[1]) return match[1];
  
  // If no URL pattern, assume it was the ID itself (stripped of any surrounding slashes)
...
export async function readSheet(tabName) {
  ...
  try {
    res = await fetch(url, {
      headers: {
        "User-Agent": "Mozilla/5.0 (Vercel Serverless; ForexPortal)",
...
// Fallback: Parses the "date" column if it exists.
export function monthNum(row) {
  const direct = num(row.month_number ?? row.monthnumber ?? row.monthno, NaN);
  ...
  const dateStr = String(row.date || row.requestdate || "").trim();
  if (dateStr) {
...
/**
 * Safely filters out sample/demo rows from the investors list.
 * Preserves "TRUE" if it's the only one left for testing.
 */
export function filterInvestors(rows) {
  if (!Array.isArray(rows)) return [];

  const EXCLUDED_USERNAMES = ["sample.investor", "admin"];
  // We keep "TRUE" (Stout001) per user instruction if it maintains a working test flow.

  return rows.filter((r) => {
    const user = String(r.portal_username ?? r.portalusername ?? r.username ?? "").trim().toLowerCase();
    const id = String(r.investorsinvestorid ?? r.investorid ?? r.id ?? "").trim().toLowerCase();
    
    // Explicitly exclude clearly sample/test rows
    if (EXCLUDED_USERNAMES.includes(user)) return false;
    if (id === "inv001" || id === "admin01") return false;
