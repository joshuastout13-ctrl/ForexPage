...
import { getFundAccountingDate } from "./month-state.js";
 ...
 */
export async function syncOpenMonthlyReturn(grossMonthPctStr, asOfDate = null) {
  if (!supabase) return;
  ...
  // Use fund accounting timezone (America/Los_Angeles) — authoritative source
  const { monthNumber: currentMonthIdx, year: currentYear } = getFundAccountingDate(asOfDate);
