...
export async function getMyfxbookLive() {
  ...
  if (CONFIG.zenrowsApiKey) {
    try {
      ...
      result.source = "Myfxbook (ZenRows)";
      
      // PERSIST TO DATABASE
      const { updateLivePerformance } = await import("./supabase.js");
      await updateLivePerformance(zenrowsData);
      
      console.log("[Myfxbook] Real-time periodic metrics merged and synced to Supabase");
    } catch (err) {
      console.error("[Myfxbook] ZenRows scraper failed:", err.message);
    }
  }
...
async function fetchZenRowsMetrics() {
  ...
  const extract = (label) => {
    // Looks for "Label</td>" followed by the first percentage value inside a span
