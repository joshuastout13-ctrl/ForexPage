...
export async function getMyfxbookLive({ previewMode = false } = {}) {
  ...
  try {
    ...
    sourceData.forEach((row) => {
      ...
      const val = fmt(row.value || row.Value || row.valuepct || "0.00%");
    ...
    console.log(`[Myfxbook] Initialized metrics from ${sourceName}`);
  } catch (err) {
  ...
  if (CONFIG.zenrowsApiKey && (!CONFIG.scrapedotdoToken || result.source !== "Myfxbook (Scrape.do)")) {
    ...
    try {
      ...
      // ... (rest of ZenRows logic if still needed, but we'll prioritize Scrape.do)
    } catch (err) {
